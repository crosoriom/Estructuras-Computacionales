...
{
// This calculation is more efficient than a long if/else chain.
    // It relies on the GPIO base addresses being spaced 0x400 apart.
    if (GPIOx >= GPIOA && GPIOx <= GPIOH) {
        return (GPIOx - GPIOA) / 0x400;
    }
...
void gpio_init(const gpio_config_t *config)
{
    ...
    GPIOx->MODDER &= ~(3U << (2 * pin));
    GPIOx->MODDER |= (mode << (2 * pin));
...
void gpio_read_pin(gpio_t *GPIOx, uint8_t pin)
{
