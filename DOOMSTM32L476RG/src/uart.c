...
void usart_init(const usart_t *config, uint32_t pclk_freq)
{
    ...
    USARTx->CR1 &= ~USART_CR_UE;                        // Disable USART first to allow configuration

    // Set baud rate. BRR = PCLK / Baudrate.
    // For an 80MHz clock with no APB prescaler, PCLK is 80MHz.
    usart_port->BRR = pclk_freq / config->baudrate;

    usart_set_word_lenght(USARTx, config->WORD_LENGHT);
    ...
    usart_port->CR1 |= USART_CR1_TE | USART_CR1_RE;
